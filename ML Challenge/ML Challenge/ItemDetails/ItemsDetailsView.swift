...
class ItemsDetailsView: UIViewController, ItemDetailsViewProtocol {
    ...
    let lblAvilableItem: UILabel = {
        ...
        lblAvilableItem.numberOfLines = 0
    ...
    func setInfo() {
        if let urlString = produt?.pictures[0].url, let urlImg = URL(string: urlString) {
        imgPrinciaplImage.setImage(url:urlImg, placeholder: UIImage(named: "Item"))
        }
        if let nameProdut = produt?.name {
            lblNameItem.text = nameProdut
        }
        if let valueOne = produt?.attributes[0].name, let valueTwo = produt?.attributes[0].valueName {
            lblModelItem.text = "\(valueOne): \(valueTwo) "
        }
    }
    ...
    func setUpConstrains() {
        ...
        NSLayoutConstraint.activate([
            ...
            stkInfoItem.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 10),
            stkInfoItem.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -10),
